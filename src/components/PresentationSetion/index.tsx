...
export const PresentationSetion = () => {
  ...
  const { sendData, mockedPresentationSetion } = usePresentationSetion();
  ...
  return (
    <Flex direction="column" border={authenticated ? "1px dashed #ccc" : ""} borderRadius={4}>
      <SimpleGrid id="home" columns={2} spacing={5} w="100%">
        <LabelForImage labelRef="bannerHome" labelWidth="100%" image={imagem1} />
      ...
      <BiographyPreview imageTop={imagem2} imageBottom={imagem3} descriptionValue={description} />

      {authenticated && (
        <Flex alignSelf={{ sm: "center", lg: "start" }} gap={5}>
          <Button size={{ md: "md", xl: "lg" }} colorScheme="gray">
            Cancelar
          </Button>
